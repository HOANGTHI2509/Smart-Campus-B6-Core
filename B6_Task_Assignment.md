...
- [ ] **Task 1.1:** Khởi tạo project FastAPI, thiết lập cấu trúc thư mục, viết `Dockerfile` và `docker-compose.yml` (Chạy tất cả trên 1 máy ảo).
- [ ] **Task 1.2:** Thiết lập file `.env` chứa địa chỉ IP của các nhóm khác. Đảm bảo app FastAPI cấu hình host `0.0.0.0:8000`.
- [ ] **Task 1.3:** Viết API `GET /health` (bắt buộc theo chuẩn của thầy).
- [ ] **Task 1.4:** Viết API Inbound Router `POST /ai/events` (Webhook để B4 đẩy báo động khói/cháy).
- [ ] **Task 1.5:** Viết API Inbound Router `POST /access/check` (Nhận UID từ B3 xin mở cổng).
- [ ] **Task 1.6:** Viết các class Outbound Client (dùng thư viện `httpx`) để chủ động gọi sang B4 (Face Match), gọi B7 (Báo động), gọi B3 (Ra lệnh mở cổng).
...
- [ ] **Task 2.1:** Dùng thư viện Python `paho-mqtt` thiết lập kết nối với HiveMQ Cloud bằng Account IoT cấp (Đọc từ file `.env`).
- [ ] **Task 2.2:** Viết hàm Subscribe lắng nghe topic `smart-campus/events/sensor` của nhóm B1 (IoT).
- [ ] **Task 2.3:** Viết Business Logic Engine (Xử lý Policy): Phân tích cú pháp JSON từ IoT, check ngưỡng nhiệt độ, khói. Nếu nguy hiểm -> Gọi module Outbound của Leader để bắn lệnh sang nhóm B7.
- [ ] **Task 2.4:** Subscribe lắng nghe topic `smart-campus/events/camera` của nhóm B2. Bắt các event phát hiện chuyển động.
- [ ] **Yêu cầu Minh chứng:** Cài đặt log xịn (ví dụ: thư viện `loguru`). Mỗi khi MQTT nhận tin nhắn, in JSON có định dạng màu mè ra màn hình console để show cho giảng viên chấm.
